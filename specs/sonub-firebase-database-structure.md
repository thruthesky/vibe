...
├── user-stats/                     # 사용자별 상세 통계 (기간별)
├── user-following/                 # 내가 팔로우한 사용자
├── user-followers/                 # 나를 팔로우한 사용자
├── user-feed/                      # 사용자 피드 (팔로우한 사람들의 메시지)
├── my-actions/                     # 나의 활동 히스토리
├── received-reactions/             # 받은 반응 히스토리
├── posts/                          # 게시글
├── comments/                       # 댓글
├── post-rankings/                  # 게시글 순위 (인기글)
├── influencer-scores/              # 인플루언서 점수
├── influencer-rankings/            # 인플루언서 순위
├── likes/                          # 사용자별 좋아요 목록
├── likes-by/                       # 좋아요한 사용자 목록 (역참조)
├── chat-rooms/                     # 채팅방 정보
├── chat-room-passwords/            # 채팅방 비밀번호
├── chat-messages/                  # 채팅 메시지
├── chat-message-comments/          # 채팅 메시지 댓글
├── chat-joins/                     # 사용자별 채팅방 참여 정보
├── chat-invitations/               # 채팅 초대
├── chat-favorites/                 # 채팅 즐겨찾기 폴더
...
└── settings/
    └── admins/
        ├── <uid>: true                   # 관리자 UID (Cloud Functions)
        └── <uid>: true
```

### 클라이언트/서버 역할 분리
- ✅ **Cloud Functions만 관리**: 관리자 추가/제거
- ✅ **클라이언트 읽기**: 관리자 권한 확인
...
/status/
└── <userId>/
    ├── online: true                      # 온라인 여부 (boolean)
    └── lastSeen: 1698473000000           # 마지막 접속 시간 (밀리초 타임스탬프)
```
...
| 필드 | 타입 | 필수 | 저장 주체 | 설명 |
|------|------|------|----------|------|
| `online` | boolean | ✅ | 클라이언트 | 온라인 여부 (presence 시스템) |
| `lastSeen` | number | ✅ | 클라이언트 | 마지막 접속 시간 (타임스탬프) |

### 클라이언트/서버 역할 분리
- ✅ **클라이언트**: Firebase Presence 시스템으로 자동 관리
- ✅ **읽기 전용**: 다른 사용자의 온라인 상태 확인
...
└── <uid>/
    ...
    ├── coverPhotoUrl: "https://..."              # 커버 사진 URL (클라이언트)
    ├── bio: "안녕하세요"                         # 자기소개 (클라이언트)
...
| `coverPhotoUrl` | string | ❌ | 클라이언트 | 커버 사진 URL |
| `bio` | string | ❌ | 클라이언트 | 자기소개 |
...
| `languageCode` | string | ❌ | 클라이언트 | 사용자 언어 코드 (예: ko, en, ja, zh) |
| `createdAt` | number | ✅ | Cloud Functions | 계정 생성 시간 (타임스탬프) |
| `updatedAt` | number | ✅ | Cloud Functions | 프로필 수정 시간 (타임스탬프) |
| `lastLoginAt` | number | ❌ | Cloud Functions | 마지막 로그인 시간 (타임스탬프) |
...
## 사용자별 상세 통계 (user-stats)

사용자별 기간별 상세 통계를 저장합니다.
...
└── <uid>/
    ├── total/                                    # 전체 통계
    │   ├── createdPosts: 10                     # 작성한 게시글 수
    │   ├── createdComments: 50                  # 작성한 댓글 수
    │   ├── receivedPostComments: 25             # 게시글에 받은 댓글 수
    │   ├── receivedCommentReplies: 15           # 댓글에 받은 대댓글 수
    │   ├── receivedComments: 40                 # 받은 모든 댓글 수
    │   ├── receivedLikes: 100                   # 받은 좋아요 수
    │   └── receivedFollowers: 30                # 받은 팔로워 수
    ├── daily/                                    # 일별 통계
    │   └── <yyyyMMdd>/
    │       ├── createdPosts: 2
    │       └── ...
...
| 타입 | 설명 |
|------|------|
| `createdPosts` | 사용자가 작성한 게시글 수 |
| `createdComments` | 사용자가 작성한 댓글 수 |
| `receivedPostComments` | 게시글이 받은 댓글 수 |
| `receivedCommentReplies` | 댓글이 받은 대댓글 수 |
| `receivedComments` | 받은 모든 댓글 수 (게시글 댓글 + 대댓글) |
...
/posts/
└── <postId>/
    ├── authorUid: "author-uid"                # 작성자 UID (클라이언트)
    ├── category: "discussion"                 # 카테고리 (클라이언트)
    ├── title: "게시글 제목"                   # 제목 (클라이언트)
    ├── text: "게시글 내용"                    # 내용 (클라이언트)
    ├── urls: {0: "https://...", 1: "..."}     # 첨부파일 URL 목록 (클라이언트)
    ├── createdAt: 1698473000000               # 작성 시간 (Cloud Functions)
    ├── updatedAt: 1698474000000               # 수정 시간 (클라이언트)
    ├── editedAt: 1698475000000                # 마지막 수정 시간 (클라이언트)
    ├── deleted: false                         # 삭제 여부 (클라이언트)
    ...
    ├── childCount: 5                          # 첫 레벨 댓글 수 (Cloud Functions)
    ├── totalChildCount: 15                    # 모든 댓글 수 (Cloud Functions)
    └── reportCount: 0                         # 신고 개수 (Cloud Functions)
```
...
| 필드 | 타입 | 필수 | 저장 주체 | 설명 |
|------|------|------|----------|------|
| `authorUid` | string | ✅ | 클라이언트 | 작성자 UID |
| `category` | string | ✅ | 클라이언트 | 카테고리 (discussion, qna, news 등) |
| `title` | string | ✅ | 클라이언트 | 게시글 제목 |
| `text` | string | ❌ | 클라이언트 | 게시글 내용 |
| `urls` | Record<number, string> | ❌ | 클라이언트 | 첨부파일 URL 목록 |
| `createdAt` | number | ✅ | Cloud Functions | 작성 시간 (타임스탬프) |
| `updatedAt` | number | ❌ | 클라이언트 | 수정 시간 |
| `editedAt` | number | ❌ | 클라이언트 | 마지막 수정 시간 |
| `deleted` | boolean | ❌ | 클라이언트 | 삭제 여부 (소프트 삭제) |
| `deletedAt` | number | ❌ | 클라이언트 | 삭제 시간 |
...
| `childCount` | number | ✅ | Cloud Functions | 첫 레벨 댓글 수 |
| `totalChildCount` | number | ✅ | Cloud Functions | 모든 댓글 수 (대댓글 포함) |
| `reportCount` | number | ✅ | Cloud Functions | 신고 개수 |
...
└── <postId>/
    └── <commentId>/
        ├── authorUid: "author-uid"            # 작성자 UID (클라이언트)
        ├── text: "댓글 내용"                  # 내용 (클라이언트)
        ├── urls: {0: "https://..."}           # 첨부파일 URL 목록 (클라이언트)
        ├── parentId: null                     # 부모 댓글 ID (클라이언트, 최상위는 null)
        ├── createdAt: 1698473000000           # 작성 시간 (클라이언트)
        ├── editedAt: null                     # 수정 시간 (클라이언트)
        ├── deleted: false                     # 삭제 여부 (클라이언트)
        ├── deletedAt: null                    # 삭제 시간 (클라이언트)
        ├── listOrder: "001"                   # 계층적 정렬 필드 (클라이언트)
        ├── order: -1698473000000              # 정렬 필드 (Cloud Functions)
...
|------|------|------|----------|------|
| `authorUid` | string | ✅ | 클라이언트 | 작성자 UID |
| `text` | string | ✅ | 클라이언트 | 댓글 내용 |
| `urls` | Record<number, string> | ❌ | 클라이언트 | 첨부파일 URL 목록 |
| `parentId` | string \| null | ✅ | 클라이언트 | 부모 댓글 ID (최상위는 null) |
| `createdAt` | number | ✅ | 클라이언트 | 작성 시간 (타임스탬프) |
| `editedAt` | number | ❌ | 클라이언트 | 수정 시간 |
| `deleted` | boolean | ❌ | 클라이언트 | 삭제 여부 (소프트 삭제) |
| `deletedAt` | number | ❌ | 클라이언트 | 삭제 시간 |
| `listOrder` | string | ✅ | 클라이언트 | 계층적 정렬 필드 (예: "001", "001-01") |
| `order` | number | ✅ | Cloud Functions | 정렬 필드 (-createdAt) |
| `likeCount` | number | ✅ | Cloud Functions | 좋아요 개수 |
| `childCount` | number | ✅ | Cloud Functions | 직접 자식 댓글 수 |
...
└── <roomId>/
    ...
    │   "uid2": false
    │ }
    ├── password: true                         # 비밀번호 활성화 플래그 (클라이언트)
...
**채팅방 생성 시:**
- 클라이언트: `type`, `name`, `description`, `owner`, `members`, `password`
...
## 채팅 즐겨찾기 폴더 (chat-favorites)

채팅방 즐겨찾기 폴더는 `/chat-favorites/<uid>/<favoriteId>/` 경로에 저장됩니다.
...
└── <uid>/
    └── <favoriteId>/
        ├── id: "favorite-123"                # 즐겨찾기 ID (클라이언트)
        ├── name: "중요 채팅"                  # 폴더 이름 (클라이언트)
        ├── description: "업무 관련 채팅"      # 폴더 설명 (클라이언트)
        ├── createdAt: 1698473000000          # 생성 시간 (클라이언트)
        ├── folderOrder: "-1698473000000"     # 정렬 순서 (클라이언트)
        └── roomList: {                       # 폴더에 포함된 채팅방 (클라이언트)
            "room-id-1": true,
            "room-id-2": true
        }
```
...
| 필드 | 타입 | 필수 | 저장 주체 | 설명 |
|------|------|------|----------|------|
| `id` | string | ✅ | 클라이언트 | 즐겨찾기 폴더 ID |
| `name` | string | ✅ | 클라이언트 | 폴더 이름 |
| `description` | string | ❌ | 클라이언트 | 폴더 설명 |
| `createdAt` | number | ✅ | 클라이언트 | 폴더 생성 시간 |
| `folderOrder` | string | ✅ | 클라이언트 | 정렬 순서 (-createdAt) |
| `roomList` | Record<string, boolean> | ✅ | 클라이언트 | 폴더에 포함된 채팅방 목록 |

### 관련 가이드
다른 spec 파일에서 이 구조가 필요한 경우, 이 문서를 참조하세요:
- **참조 방법**: `[데이터베이스 구조](./sonub-firebase-database-structure.md#채팅-즐겨찾기-폴더-chat-favorites)`

---

## 채팅방 비밀번호 (chat-room-passwords)

채팅방 비밀번호는 `/chat-room-passwords/<roomId>/` 경로에 저장됩니다.
...
/chat-room-passwords/
└── <roomId>/
    ├── password: "1234"                      # 평문 비밀번호 (클라이언트)
    └── try/                                  # 비밀번호 입력 시도
        └── <uid>: "1234"                     # 사용자의 입력 시도 (클라이언트)
```
...
| 필드 | 타입 | 필수 | 저장 주체 | 설명 |
|------|------|------|----------|------|
| `password` | string | ✅ | 클라이언트 | 평문 비밀번호 |
| `try/{uid}` | string | ❌ | 클라이언트 | 사용자의 비밀번호 입력 시도 |

### 보안 규칙
- 채팅방 소유자만 `password` 필드 읽기/쓰기 가능
- 일반 사용자는 `try/{본인uid}` 경로에만 쓰기 가능
- Cloud Functions에서 검증 후 자동 삭제

---

## 채팅 메시지 댓글 (chat-message-comments)

채팅 메시지에 대한 댓글은 `/chat-message-comments/<messageId>/<commentId>/` 경로에 저장됩니다.
...
└── <messageId>/
    └── <commentId>/
        ├── text: "댓글 내용"                 # 댓글 내용 (클라이언트)
        ├── urls: {0: "https://..."}          # 첨부파일 URL 목록 (클라이언트)
        ├── authorUid: "author-uid"           # 작성자 UID (클라이언트)
        ├── parentId: null                    # 부모 댓글 ID (클라이언트)
        ├── createdAt: 1698473000000          # 작성 시간 (클라이언트)
        ├── listOrder: "001"                  # 계층적 정렬 필드 (클라이언트)
        ├── editedAt: null                    # 수정 시간 (클라이언트)
        ├── deleted: false                    # 삭제 여부 (클라이언트)
        ├── deletedAt: null                   # 삭제 시간 (클라이언트)
        ├── childCount: 2                     # 자식 댓글 수 (Cloud Functions)
        └── likeCount: 5                      # 좋아요 개수 (Cloud Functions)
```
...
| `listOrder` | string | ✅ | 클라이언트 | 계층적 정렬 필드 |
| `editedAt` | number | ❌ | 클라이언트 | 수정 시간 |
| `deleted` | boolean | ❌ | 클라이언트 | 삭제 여부 |
| `deletedAt` | number | ❌ | 클라이언트 | 삭제 시간 |
| `childCount` | number | ✅ | Cloud Functions | 자식 댓글 수 |
| `likeCount` | number | ✅ | Cloud Functions | 좋아요 개수 |
...
## 인플루언서 점수 (influencer-scores)

인플루언서 점수는 `/influencer-scores/<uid>/` 경로에 저장됩니다.

### 데이터 구조

```
/influencer-scores/
└── <uid>: 1000                               # 인플루언서 점수 (Cloud Functions)
```
...
## 인플루언서 순위 (influencer-rankings)

인플루언서 순위는 `/influencer-rankings/<period>/<date>/<uid>/` 경로에 저장됩니다.
...
### 특징
- 점수를 음수로 저장하여 오름차순 정렬 시 고득점자가 먼저 표시됨
- Cloud Functions에서 `influencer-scores` 변경 시 자동 업데이트

### 클라이언트/서버 역할 분리
- ✅ **Cloud Functions만 관리**: 점수 변경 시 자동 업데이트
- ❌ 클라이언트는 읽기 전용

---

## 나의 활동 히스토리 (my-actions)

사용자가 수행한 모든 활동 기록은 `/my-actions/<uid>/<pushKey>/` 경로에 저장됩니다.
...
└── <uid>/
    └── <pushKey>/
        ├── type: "like"                      # 활동 타입 (Cloud Functions)
        ├── targetType: "post"                # 대상 타입 (Cloud Functions)
        ├── targetId: "post-123"              # 대상 ID (Cloud Functions)
        └── createdAt: 1698473000000          # 활동 시간 (Cloud Functions)
```
...
- `comment`: 댓글
- `user`: 사용자
- `chat-message-{roomId}`: 채팅 메시지

### 클라이언트/서버 역할 분리
- ✅ **Cloud Functions만 관리**: 활동 발생 시 자동 기록
- ❌ 클라이언트는 읽기 전용

---

## 받은 반응 히스토리 (received-reactions)

다른 사용자로부터 받은 반응 기록은 `/received-reactions/<uid>/<pushKey>/` 경로에 저장됩니다.
...
└── <uid>/
    └── <pushKey>/
        ├── fromUid: "follower-uid"           # 반응한 사용자 UID (Cloud Functions)
        ├── type: "like"                      # 반응 타입 (Cloud Functions)
        ├── targetType: "post"                # 대상 타입 (Cloud Functions)
        ├── targetId: "post-123"              # 대상 ID (Cloud Functions)
        ├── postId: "post-123"                # 게시글 ID (댓글인 경우, Cloud Functions)
        └── createdAt: 1698473000000          # 반응 시간 (Cloud Functions)
```
...
### 클라이언트/서버 역할 분리
- ✅ **Cloud Functions만 관리**: 반응 발생 시 자동 기록
- ❌ 클라이언트는 읽기 전용
