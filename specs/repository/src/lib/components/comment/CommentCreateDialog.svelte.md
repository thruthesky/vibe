...
<script lang="ts">
	...
	async function handleSave(
		...
		if (!text.trim() && Object.keys(urls).length === 0) {
			return { success: false, error: m.commentContentOrAttachmentRequired() };
		}
		...
		try {
			...
			return { success: false, error: m.commentSaveFailed() };
		}
	}
...
<MessageEditModal
	bind:open
	title={parentId ? m.commentReplyTitle() : m.commentCreateTitle()}
	textLabel=""
	roomId={postId}
...
>
	...
	{#if parentId && parentText}
		<div class="parent-comment-info">
			<div class="parent-comment-label">{m.commentReplyTargetLabel()}</div>
