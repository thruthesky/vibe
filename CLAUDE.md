...
- [ ] **6단계**: 🚨🚨🚨 즉시 배포 🚨🚨🚨 **🔥 코드 수정 후 가장 먼저 실행! 🔥**
  - **⚡ 배포 명령어 (로컬 빌드 후 업로드 - 가장 빠름):**

    **1단계: 로컬에서 빌드 (Vercel 규격 산출)**
    ```bash
    npx vercel@latest build
    ```
    - 로컬 컴퓨터에서 프로덕션 규격으로 빌드
    - `.vercel` 디렉토리에 빌드 결과물 생성

    **2단계: 빌드된 결과물만 업로드 → 즉시 프로덕션 배포**
    ```bash
    npx vercel@latest deploy --prebuilt --prod --yes
    ```
    - `--prebuilt`: 로컬 빌드 결과물을 사용 (서버에서 빌드 생략)
