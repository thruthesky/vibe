...
- [ ] **5단계**: 로컬 테스트 및 커밋 (개발자 배포 요청 전)
  ...
  **🚀 배포 요청 전에 다음 작업들을 수행합니다:**
  ...
  #### 5-2. UTF-8 인코딩 검증 (문서/코드 수정/생성 시 필수!)
...
- [ ] **7단계**: 빌드 날짜/버전 업데이트 ⚠️ **개발자가 명시적으로 요청한 경우에만 실행**
...
3. **UTF-8 인코딩 검증**
4. **git commit & push**
5. **개발자 배포 요청 대기** ← 배포는 이 때 하지 않음!
6. **개발자가 "배포"라고 입력** → 그 때 배포 실행
7. **배포 완료 후 검증** (필요시)
