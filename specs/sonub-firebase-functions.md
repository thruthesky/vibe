...
- 댓글의 부모 메시지 식별을 위해 `/comment-locations/{commentId}` 활용
...
- **사용자**: `/users/{uid}`
   - **좋아요 경로**: `/likes/{uid}/{targetId}`
     - 값: `"message"` 또는 `"comment"` 문자열
     - `targetId`: 게시글 `messageId` 또는 댓글 `commentId`
     - 댓글의 부모 `messageId`는 `/comment-locations/{commentId}` 맵을 통해 조회
- **댓글**: `/comments/{commentId}`
...
export const onLikeCreated = onValueCreated("/likes/{uid}/{targetId}", async (event) => {
  ...
  if (targetType === "message") {
    ...
    const messageId = await admin.database().ref(`comment-locations/${targetId}`).once("value");
    // ... 이후 로직 계속
  }
...
```
  like.handler - 좋아요 처리
    ...
    ✓ 댓글 좋아요 취소 시 comment-locations를 조회하여 감소

  onLikeCreated
    ✓ /likes/{uid}/{targetId} → message 타입 처리
    ✓ /likes/{uid}/{targetId} → comment 타입 처리
...
describe("like.handler - 좋아요 증감", () => {
  ...
  it("댓글 좋아요 취소 시 comment-locations를 조회한다", async () => {
    onceStub.resolves({exists: () => true, val: () => "message-777"});
    await handleLikeDelete("user-2", "comment-9", "comment");
