...
### v1.2.0 (2025-01-17)
- 게시글 클릭 동작 제거
  - `post-content` 영역의 onclick 핸들러 제거
  - 게시글 클릭으로 채팅방으로 이동하는 기능 제거
  - CSS에서 `cursor-pointer` 제거
- 채팅방 이름 표시 추가
  - `post-header-right`에 채팅방 이름 표시 (카테고리와 시간 사이)
  - 최대 8글자로 제한, 초과 시 ellipsis(`...`) 표시
  - `getChatRoomName()` 함수 추가 (`src/lib/functions/chat.functions.ts`)
  - `/chat-rooms/{roomId}/name` 필드만 읽어 비용 최적화
- CSS 스타일 추가
  - `post-room-name` 클래스 추가: 채팅방 이름 스타일링
- 문서 업데이트
  - HTML 구조 업데이트 (채팅방 이름 포함)
  - 상호작용 섹션 업데이트 (채팅방 이름 표시 설명)
  - 코드 예시 업데이트

### v1.1.0 (2025-01-17)
- 게시글 카드 레이아웃 재구성
  - 작성자 프로필을 상단 왼쪽으로 이동
  - 카테고리와 시간을 상단 오른쪽으로 이동
  - 하단 액션 바 추가 (좋아요, 댓글, 수정, 삭제)
  - 수정/삭제를 아이콘 전용 버튼으로 변경
- HTML 구조 업데이트
  - post-header: 작성자 프로필 + 카테고리 + 시간
  - post-content: 게시글 내용 (클릭 가능)
  - post-actions: 하단 액션 바 (좋아요, 댓글, 수정, 삭제)
- CSS 스타일링 업데이트
  - 새로운 클래스 추가: post-header, post-header-right, post-actions, post-actions-left, post-actions-right
  - 액션 버튼 클래스 추가: action-button, action-icon-button, action-icon-button-danger
- 상호작용 개선
  - 게시글 클릭 영역을 post-content로 제한
  - 액션 버튼에 stopPropagation 적용
  - SVG 아이콘 사용
...
- 게시글 목록 HTML 구조 정의
- UserProfile 컴포넌트 적용
- FileAttachments 컴포넌트 개선 사항 반영
- Tailwind CSS 스타일링 규칙 정의
- 접근성 및 성능 최적화 가이드라인 추가
