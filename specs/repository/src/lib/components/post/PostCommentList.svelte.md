...
async function handleDeleteComment(commentId: string) {
	if (!confirm(m.commentDeleteConfirm())) {
		return;
	}
...
<div class="post-actions">
	<div class="comment-info">
		{#if totalChildCount > 0}
			<span class="comment-count"> 💬 {m.commentCountLabel({ count: totalChildCount })} </span>
			...
			{/if}
		{:else}
			<span class="comment-count-empty">{m.commentListEmpty()}</span>
		{/if}
	</div>
	<Button
		variant="ghost"
			...
			<div class="comment-item" style="margin-left: {comment.depth * 24}px;">
				...
				{#if comment.deleted}
					<div class="comment-deleted">
						<p class="comment-deleted-text">{m.commentDeleted()}</p>
					</div>
				{:else}
					<!-- 댓글 내용 -->
