...
/**
 * sonub-firebase-database-structure.md 업데이트 스크립트
 *
 * 실제 소스 코드 기반으로 데이터베이스 구조를 정확히 반영합니다.
 */

const fs = require('fs');
const path = require('path');

const SPEC_FILE = path.join(__dirname, '../specs/sonub-firebase-database-structure.md');
const BACKUP_FILE = path.join(__dirname, '../specs/sonub-firebase-database-structure.md.backup');

// 백업 생성
console.log('📦 기존 파일 백업 중...');
fs.copyFileSync(SPEC_FILE, BACKUP_FILE);
...
- [Sonub Cloud Functions 핸들러 코드](../firebase/functions/src/handlers/)
- [Sonub 타입 정의](../firebase/functions/src/types/index.ts)
...
console.log('✍️  새로운 데이터베이스 구조 스펙 작성 중...');
fs.writeFileSync(SPEC_FILE, content, 'utf8');
console.log(`✅ 파일 업데이트 완료: ${SPEC_FILE}\n`);

console.log('📊 요약:');
console.log('   - 실제 소스 코드 기반으로 작성');
console.log('   - registerOrder 필드 추가');
console.log('   - 정렬 필드들 (sort_*) 추가');
console.log('   - displayNameLowerCase 필드 추가');
console.log('   - user-action-counters 노드 추가');
console.log('   - 모든 필드에 저장 주체 명시 (클라이언트 vs Cloud Functions)');
console.log('   - 다른 spec 파일에서 참조할 수 있도록 구조화');
console.log('');
console.log('✅ 완료! sonub-firebase-database-structure.md가 업데이트되었습니다.');
