...
- [ ] **6단계**: 🚨🚨🚨 Preview 배포 (완료될 때까지 기다림) 🚨🚨🚨 **🔥 코드 수정 후 가장 먼저 실행! 🔥**
...
- [ ] **7단계**: UTF-8 인코딩 검증 (문서/코드 수정/생성 시 필수!) ⚠️ **배포 완료 후 실행**
  - **⚡ 배포가 완료된 후 진행합니다**
...
- [ ] **8단계**: 최종 변경사항 커밋 및 푸시 ⚠️ **UTF-8 확인 후 실행**
  - **⚡ UTF-8 인코딩 검증을 완료한 후 진행합니다**
...
- [ ] **10단계**: 빌드 날짜/버전 업데이트 ⚠️ **개발자가 명시적으로 요청한 경우에만 실행**
...
3. **UTF-8 인코딩 검증** (배포 완료 후)
4. **git commit & push** (UTF-8 확인 후)
5. **Preview URL에서 테스트** (배포 완료 후)
6. **버전 업데이트** (개발자 명시적 요청 시에만)
