...
- **직접 WAF 구축하지 않음**: 자체 WAF를 추가로 만들지 않고, Cloudflare WAF를 그대로 사용한다. Cloudflare WAF가 원하지 않는 UI를 사용자에게 노출하여 아쉽지만, 현재로서는 가장 깔끔하고 빠르게 악성 요청을 차단할 수 있는 방법이므로 이를 표준으로 채택한다.
