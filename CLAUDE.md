...
**🔥 배포 실행 조건**:
- **✅ 배포 실행**: `*.tsx`, `*.ts`, `*.js`, `*.css` 등 **소스 코드 파일이 수정된 경우**
- ❌ **배포 건너뛰기**: `*.md` 문서 파일**만** 수정된 경우 (배포하지 않음!)
- **예시**:
  - ✅ app/page.tsx 수정 → 배포 실행
  - ✅ lib/chat.ts 수정 → 배포 실행
  - ❌ docs/chat.md 수정만 → 배포 하지 않음
  - ✅ docs/chat.md + app/page.tsx 둘 다 수정 → 배포 실행
