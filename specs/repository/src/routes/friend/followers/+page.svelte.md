...
original_path: src/routes/friend/followers/+page.svelte
category: route
file_type: svelte
status: current
...
**원본 경로**: `src/routes/friend/followers/+page.svelte`
...
<script lang="ts">
	 ...
	 * 팔로워 목록 페이지
	 *
	 * 현재 사용자를 팔로우하는 사용자 목록을 표시합니다.
	 * - `/user-followers/{myUid}` 경로에서 목록 조회
	 * - 실시간 업데이트 지원
	...
	let followersList = $state<UserInfo[]>([]);
	...
	async function subscribeFollowers() {
		...
		unsubscribe = onValue(
			...
			async (snapshot) => {
				...
				followersList = users.filter((u) => u !== null) as UserInfo[];
				loading = false;
			},
			(error) => {
				console.error('팔로워 목록 구독 오류:', error);
				loading = false;
			}
		);
	}
...
<div class="followers-page">
	<div class="page-header">
		<h1 class="page-title">{m.followersList()}</h1>
	</div>
	...
	{:else}
		<div class="user-list">
			{#each followersList as user (user.uid)}
				<div class="user-item">
